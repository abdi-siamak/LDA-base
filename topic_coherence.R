...
coherence <- numeric(K)
for (k in 1:K) {
  top_words <- TopPHI[(1:numberOfTopWords),k]
  coherence[k] <- topic_coherence(top_words, document_term_matrix, vocabulary = V,
                  numeric_top_words = FALSE, Top= length(top_words))
}
averageCoherence <- sum(coherence)/K
cat(coherence)
cat(averageCoherence)
